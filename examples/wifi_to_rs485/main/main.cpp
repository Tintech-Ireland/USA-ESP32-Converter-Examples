//
// wifi_to_rs485 - transparent WiFi (TCP) <-> RS485 bridge for the USA2 board.
//
// The board comes up as a WiFi soft-AP and listens on a TCP port. Bytes received
// on the TCP connection are written verbatim to the RS485 UART, and bytes read
// from the bus are sent verbatim back over TCP. No framing/protocol is imposed
// (raw <-> raw), so any plain TCP client works:
//
//     1. Join WiFi SSID "USA2-RS485" (password below).
//     2. Connect a TCP client to 192.168.4.1:3336, e.g.  nc 192.168.4.1 3336
//     3. Whatever you send goes out on the RS485 bus; whatever arrives from the
//        bus comes back to your TCP client.
//
// RS485 link: UART1 via the on-board SP3485EN (TX=GPIO9, RX=GPIO7), 115200 8N1,
// half-duplex. The transceiver's ~RE is hard-wired enabled and DE is driven by
// RS485_TXEN (GPIO10), active-HIGH. We use UART_MODE_RS485_HALF_DUPLEX, so the
// UART driver raises DE (its RTS pin) high while transmitting and drops it low
// otherwise - the correct polarity for this board - and suppresses the local echo
// of our own transmission. RS485 is a shared multi-drop bus: this bridge is
// transparent, so any addressing/arbitration is up to the bytes you send.
//
...
constexpr char TAG[] = "wifi_rs485";

// --- WiFi soft-AP settings ---
constexpr char     AP_SSID[]    = "USA2-RS485";
constexpr char     AP_PASS[]    = "usa2rs485";   // >= 8 chars for WPA2; "" = open
constexpr uint8_t  AP_CHANNEL   = 1;
constexpr uint8_t  AP_MAX_CONN  = 4;

// --- TCP server ---
constexpr uint16_t TCP_PORT     = 3336;
...
void uart_init()
{
    ...
    // RS485_DE (GPIO10) is the RTS pin; the RS485 half-duplex mode drives it high
    // during TX and low otherwise (active-HIGH DE - correct for this board).
...
void bridge_client(int sock)
{
    ...
    uint8_t buf[kIoBuf];
    for (;;) {
        // TCP -> RS485. In half-duplex mode the driver auto-asserts DE for the
        // duration of the write, then releases the bus.
        ...
        // RS485 -> TCP. The 10 ms read timeout also paces this loop.
...
void tcp_server_task(void*)
{
    ...
    for (;;) {
        ...
        ESP_LOGI(TAG, "client %s connected - bridging to RS485", ip);
