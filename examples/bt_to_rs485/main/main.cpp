//
// bt_to_rs485 - transparent Bluetooth LE <-> RS485 bridge for the USA2 board.
//
...
//   * writes bytes to the RX characteristic  -> forwarded to the RS485 bus
//   * subscribes to the TX characteristic     -> receives RS485 bus data as notifications
//
// NUS UUIDs (128-bit):
...
//
// Raw bytes both ways, no framing (the byte-stream twin of wifi_to_rs485). BLE
...
// auto-toggles it and suppresses the local TX echo. RS485 is a shared multi-drop
// bus, so any addressing is up to the bytes you send.
...
constexpr char TAG[]      = "bt_rs485";
constexpr char DEV_NAME[] = "USA2-BT-RS485";
...
void uart_init()
{
    ...
    // RS485_DE (GPIO10) is the RTS pin; RS485 half-duplex mode drives it high during
    // TX and low otherwise (active-HIGH DE - correct for this board).
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "bt_to_rs485 up: NUS <-> RS485 half-duplex (TX=GPIO%d RX=GPIO%d DE=GPIO%d) @%d 8N1",
